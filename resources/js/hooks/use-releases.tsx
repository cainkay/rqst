...
export const useReleases = ({ initialData, categories, lgas, states, search, date, trigger }: UseReleasesParams) => {
    // Fetch releases with pagination
    const fetchReleases = async ({ pageParam = 1 }) => {
        const response = await axios.get<ReleaseResponse>('/releases', {
            params: {
                page: pageParam,
